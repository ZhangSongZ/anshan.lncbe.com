...
namespace app\admin\controller\eow;

use app\common\controller\Backend;

use app\admin\model\eow\Eowexorderdetail; 
...
use app\common\controller\eoworder303;  //9810订单
use app\common\controller\eoworderBgd;  //9810报关单
/**
 * 97/98订单管理
 *
 * @icon fa fa-circle-o
 */
class Eowdeclare extends Backend
{

    /**
     * Eowdeclare模型对象
     * @var \app\admin\model\eow\Eowdeclare
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\eow\Eowdeclare;
		$this->detailModel = new \app\admin\model\eow\Eowexorderdetail;
...
file_put_contents(__DIR__ .'/time311.txt',date('Y-m-d H:i:s').base64_encode(serialize($insertdetail)).PHP_EOL, FILE_APPEND);		
		try{
			...
			//存订单详情表
			$this->saveOrderDetail($insertdetail);
...
file_put_contents(__DIR__ .'/time312.txt',date('Y-m-d H:i:s').base64_encode(serialize($filteredOrderData)).PHP_EOL, FILE_APPEND);			
			...
			if(empty($filteredOrderData['edistinate_port'])||$match1){
				$filteredOrderData['edistinate_port'] = $filteredOrderData['country_code'].'000';
			}
			...
			$filteredOrderData['order_type'] = 'W';
 file_put_contents(__DIR__ .'/time312.txt',date('Y-m-d H:i:s').base64_encode(serialize($insertorder)).PHP_EOL, FILE_APPEND);	
	...
	public function customsBgd($ids=null){
        ...
        $eoworderBgd =  new eoworderBgd();
		$eoworderBgd->GenBgd($result,$uid);
