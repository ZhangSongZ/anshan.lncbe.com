...
class eoworderBgd{
    ...
    {
        $resuFtp = db::name('merchannel')->where('auth_id',$uid)->where('status',1)->where('category','9810')->find();
        ...
        $path = date("Ymd", time());
        //如果不存在当天的目录则建立
        if (!is_dir('customsMessage/9810bgd/' . $path)) {
            mkdir('customsMessage/9810bgd/' . $path, 0777, true);
        }
        set_time_limit(0);
		foreach($result as $orderMain){
			...
			$localfile = 'customsMessage/9810bgd/' . $path . '/9810Message' . $orderMain['order_number'] . '_' . $realTime . '.xml';
    ...
    public function posts($postUrl,$postDate){
        ...
        $result = curl_exec($ch);
        curl_close($ch);
file_put_contents(__DIR__ .'/postresult.txt',' write at '.date('Y-m-d H:i:s').base64_encode(serialize($postDate)).PHP_EOL, FILE_APPEND);
file_put_contents(__DIR__ .'/postresult.txt',' write at '.date('Y-m-d H:i:s').base64_encode(serialize($result)).PHP_EOL, FILE_APPEND);
        return $result;
    }
}